...
# ==========================
library("dplyr")
df=data.frame(prop.table(table(enpClases[,1:2])))
...
# ===========================
colores=colorRampPalette(brewer.pal(9, "Oranges"))(10)
...
ggplot(niveles,aes(x=Nivel, y=Valor)) +
  ...
  guides(size=guide_legend(override.aes=list(colour=colores[seq(1,10,length.out=2)]))) + # cambia los colores de la leyenda
